...
### **3.11: The Psychological Engine**

#### **3.11.1 Design Philosophy: The Autonomous Mind**

The Psychological Engine is the complete mental and behavioral model for an autonomous character. It is a sophisticated, multi-stage component that simulates a character's internal state, their long-term goals, and their final decision-making process. It is designed to produce complex, believable, and character-driven behavior that is emergent from their experiences rather than being explicitly scripted.

The engine is composed of three distinct subcomponents that work in sequence: the **Psychological Integration** subcomponent (the "State Engine"), the **Behavioral Governor** (the "Goal Engine"), and the **Psychology Output Interface** (the "Decision Engine"). Together, they form the complete "mind" of a character.

#### **3.11.2 The Psychological Integration Subcomponent (The State Engine)**

##### **Function: Translating State into Disposition**
This subcomponent is the character's internal "mood board." It is a purely internal, background process that is constantly running. Its sole function is to consume the entirety of the character's current, context-free state from all other CSF components and, from them, calculate and maintain the character's psychological disposition.

##### **Output: The `Weighting_Matrix`**
The single, definitive output of this subcomponent is the **`Weighting_Matrix`**. This is a dictionary of `Tag: Weight` pairs that represents a snapshot of the character's current biases, moods, and priorities. It is a context-free representation of *how the character feels*, which is then consumed by the other subcomponents to make decisions.

##### **Generating the Weights: Inputs & Process**
The `Weighting_Matrix` is generated by translating the character's raw state into a series of weighted tags.

*   **Baseline Weights (The Foundation):**
    *   **From `Character Nature`:** Provides the static, baseline weights for broad behavioral categories (e.g., `[Supportive]`, `[Aggressive]`).
    *   **From `Attributes` & `Skillsets`:** Provides a subtle, constant bias towards actions that use the character's strengths (e.g., a high `Acumen` score adds a small positive weight to the `[Acumen]` tag).

*   **Dynamic State Weights (The "Mood"):**
    *   **From `Meter States`:** The 4-Stage Psychology of each meter applies powerful, global weights (e.g., Low `Vitality` applies a large positive weight to `[Restorative]` tags).
    *   **From `Anatomy` & `Eroticism Profile`:** Specific properties and affinities generate highly contextual weights (e.g., high `nipple.sensitivity` adds a weight to `[Target_ID: nipples_01]`; a high `Affinity Score` for `bondage` adds a weight to the `[Bondage]` tag).
    *   **From All Other States:** Every other CSF component (`Identity`, `Relationships`, `Agency`, `Arousal`, `Climax`, `Corruption`) provides its own set of dynamic weights that are added to the matrix, creating a complex and constantly fluctuating psychological snapshot.

*   **Modulation:** The final weights are not a simple sum. Some states modulate the weights generated by others (e.g., high `control_comfort` can reduce the weight of `[Restore_Agency]` tags), allowing for the emergence of complex psychological phenomena like accommodation and denial.

#### **3.11.3 The Behavioral Governor Subcomponent (The Goal Engine)**

##### **Function: Establishing Long-Term Strategy**
If the `Psychological Integration` subcomponent represents a character's immediate, emotional state, the Behavioral Governor represents their "higher brain"—their capacity for memory, learning, and long-term planning. Its primary function is to manage the character's persistent, high-level **`Character Goals`**, which provide strategic direction to their autonomous behavior, particularly during the exploration phase.

##### **Goal Generation and Management**
This subcomponent periodically reviews significant "catalyst events" (e.g., the end of an encounter, a major discovery, a relationship threshold change) and the character's current `Weighting_Matrix`. If it detects a new, powerful, and persistent psychological driver that aligns with the character's `core_drives` (from their `Nature`), it can generate or update a `Character Goal` object.

*   **The `Character Goal` Structure:** A goal is a simple, persistent data object that defines a high-level objective.
    ```
    Character_Goal {
      Goal_Type: "Confront_Character", // The general objective
      Target_ID: "manipulator_id",     // The target of the goal
      Motivation: "Revenge",           // The psychological driver
      Priority: 8                      // A 1-10 score for AI prioritization
    }
    ```
*   **Prioritization:** The Governor maintains a prioritized list of all active goals, ensuring the character is always pursuing their most pressing long-term objective.

##### **Integration with the `Exploration_Engine`**
The list of `Character Goals` is the primary driver of companion autonomy. The `Exploration_Engine` continuously queries this subcomponent for each companion's highest-priority goal. This provides the "why" for a companion's actions, allowing the `Exploration_Engine` to direct their movement and behavior in a purposeful, intelligent manner, which in turn leads to the emergent narrative encounters that are central to the game's design.

#### **3.11.4: The Psychology Output Interface (The Decision Engine)**

##### **Function: Translating Psychology into Action**
This subcomponent is the "public face" of the character's mind. It is a function that is called by an external engine (like the `Encounter_Engine` or `Exploration_Engine`) whenever a decision is required. Its job is to take the character's internal state and long-term goals and apply them to a specific, immediate context to produce a single, definitive action.

##### **The Decision-Making Process**
The Interface follows a strict, multi-step process when called:

1.  **Receive Context:** It receives a `Context` packet from the calling engine, which includes a list of all mechanically possible actions and potential targets.
2.  **Apply Hard Overrides:** It first filters the list of possible actions based on the character's absolute physical limitations. Actions that are impossible due to an `Agency 0` state or a `Meter Crisis` are immediately removed from consideration.
3.  **Calculate Urgency:** For every remaining action, it calculates a final `Urgency Score`. This calculation is a sophisticated function that uses the `Weighting_Matrix` (from `Psychological Integration`) and the active `Character Goals` (from the `Behavioral Governor`) as its primary inputs, applying them to the specific tags and targets of the action.
4.  **Select Final Action:** It identifies the single action with the highest `Urgency Score`.
5.  **Generate Narrative Flavor:** It performs a final check on the character's overall psychological state to generate the appropriate `Emotional Tone` and `Internal Monologue Hint` for the chosen action.
6.  **Return Output:** It packages the winning action, its score, and the narrative flavor into the final `PsychologyOutput` packet and returns it to the calling engine.

##### **Output Application: Emergent Social Dynamics**
While each character's psychological engine operates independently, the collective result of their decisions creates complex, unscripted social dynamics. The interplay of individual `Goals`, `Relationships`, and `Natures`, processed through this decision-making framework, is the primary driver of the game's emergent narrative, leading to spontaneous alliances, rivalries, and betrayals.

#### **3.11.5 The `PsychologyOutput` Packet Structure**

**[TODO: The final, detailed specification for this data structure is pending the design of the `Scene Requirement Data Structure` ("Intent Packet") in the `Narrative_Engine.md` and the finalization of context requirements from the `Exploration_Engine.md`. This section will be completed as part of the Engine design phase to ensure perfect integration.]**
